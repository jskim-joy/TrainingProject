<template>
  <div id="calendar_monthly" class="mt-3" v-if="cell_info.length > 0">
    ...
    <div class="row" v-for="row in Array(cal_info.rows).keys()" :key="row"> <!--칸 부분 -->
      <div
          ...
          <div class="">
            <span>
              <button
                ...
                :ref="
                  (el) => {
                    if (el) completed_refs[row * 7 + col] = el
                  }
                "
...
<script setup>
import { onBeforeUpdate, onUpdated, ref, watch, reactive } from '@vue/runtime-core'
import { nextTick } from 'process'

const props = defineProps([cal_info, cell_info, todos])

const days = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat']

const completed_refs = ref([])
const todos_refs = ref([])
...
const createPopOverTemplate = (type, date) => {
...
const createContent = (type, todos) => {
  let ret = '<ol class="list-group list-group-numbered m-0">'
  ...
  nextTick(() => {
    ...
    for (const key in todos_refs.value) {
      const tpopover = new Popover(todos_refs.value[key], {
        ...
        content: createContent('completed', props.cell_info[key].todos),
        template: createPopOverTemplate(
          'todos',
          props.cell_info[key].date
        ),
